...
def resgister_tools(mcp):
    ...
    async def get_race_results(
        year: int= Field(default=time.localtime().tm_year, description="The year for which the race results needs to be fetched")
        )-> Dict[str, Any]:
        """
        This endpoint is use-less
        """
        async with httpx.AsyncClient(timeout=10.0) as client:
            try:
                response = await client.get( 
                    url= cfg.BASE_URL + f'f1/{year}/results/'
                )
                if response.status_code == 200:
                    json = response.json()
                    races = json["MRData"]["RaceTable"]["Races"]
